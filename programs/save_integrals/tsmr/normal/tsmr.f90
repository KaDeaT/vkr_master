...
real :: tx(n),tx1(n),tx2(n),x((u+n+1)*(pmax+1)),a(na),aa(na),ox(np),vtb(vtn*(pmax-pmin)),rdeg,atol,rtol,&
		...
		h_0_norm, h_1_norm, x0_norm(n), x1_norm(n), modul_cord, modul_vel,  h_c0_norm(3), h_c1_norm(3)
...
modul_cord = sqrt(x0(1)**2 + x0(2)**2 + x0(3)**2)
modul_vel =  sqrt(x0(4)**2 + x0(5)**2 + x0(6)**2)

x0_norm(1) = x0(1)/modul_cord
x0_norm(2) = x0(2)/modul_cord
x0_norm(3) = x0(3)/modul_cord
x0_norm(4) = x0(4)/modul_vel
x0_norm(5) = x0(5)/modul_vel
x0_norm(6) = x0(6)/modul_vel
x0_norm(7) = x0(7)
...
h_0 = calc_he(x0, n, mass, k_planets)
h_0_norm = calc_he(x0_norm, n, mass, k_planets)
call calc_hc(x0,h_c0,mass, k_planets)
call calc_hc(x0_norm,h_c0_norm,mass, k_planets)

write(1992,*) 'h_energy0, h_enegry0_norm'
write(1992,56) h_0, h_0_norm
write(1992, *) 'h_square0, h_square0_norm'
write(1992,56) (h_c0(i), i=1,3)
write(1992,56) (h_c0_norm(i), i=1,3)
...
do  
    ...
    if((tc+h-ox(ix))*dir>0) then
        do 
			...
			x1_norm(1) = tx1(1)/modul_cord
			x1_norm(2) = tx1(2)/modul_cord
			x1_norm(3) = tx1(3)/modul_cord
			x1_norm(4) = tx1(4)/modul_vel
			x1_norm(5) = tx1(5)/modul_vel
			x1_norm(6) = tx1(6)/modul_vel
			x1_norm(7) = tx1(7) 
			
			write(1992,*)"coordinates"
			call rell_error(x0,tx1,n)
			write(1992,*)"coordinates_norm"
			call rell_error(x0_norm,x1_norm,n)
			
			h_1 = calc_he(tx1, n, mass, k_planets)
			h_1_norm = calc_he(x1_norm, n, mass, k_planets)
			...
			write(1992,*) 'h_energy_norm'
			write(1992,56) h_1_norm 
			h_error = abs(h_1_norm - h_0_norm)/abs(h_0_norm)
			write(1992,*) 'h_energy_norm error:'
			write(1992,57) h_error 
			...
			call calc_hc(x1_norm,h_c1_norm,mass,k_planets)
			write(1992, *) 'h_square_norm'
			write(1992,56) (h_c1_norm(i), i=1,3)
			write(1992,*) 'h_square_norm error:'
			call rell_error(h_c0_norm,h_c1_norm,3)
